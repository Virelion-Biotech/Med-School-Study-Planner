...
def test_adaptive_optimizer_respects_capacity_and_rest_day():
    ...
    for d in {s.date for s in plan.sessions}:
        ...
        assert all(s.planned_minutes <= 60 for s in plan.sessions if s.date == d)


def test_adaptive_optimizer_caps_each_topic_to_remaining_workload():
    start = date(2026, 8, 24)
    subjects = [Subject("a", "A")]
    topics = [Topic("t", "a", "Topic", estimated_hours=1)]
    profile = UserProfile(daily_available_minutes=180, minimum_subject_minutes_week=0, max_session_minutes=60)
    plan = optimize_adaptive_week(subjects, topics, [], profile, start, days=5)
    total = sum(s.planned_minutes for s in plan.sessions if s.topic_id == "t")
    assert total <= 60

...
def test_subject_topic_exam_profile_crud(tmp_path):
    ...
    exam = client.post('/exams', json={'id':'cardio-block','date':str(date.today()+timedelta(days=7)),'subject_ids':['cardio'],'topic_ids':['heart-failure'],'weight':1.5})
    ...
    assert len(snap['memory_states']) == 0

    assert client.delete('/topics/heart-failure').status_code == 409
    assert client.delete('/subjects/cardio').status_code == 409

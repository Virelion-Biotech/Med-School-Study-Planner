...
from .adaptive_cpsat import AdaptivePlan
...
def kc_explanation(kc_id: str):
    ...
    summary = aggregate_topic_kc_signals([state], tuple(t.id for t in mapped_topics), tuple(m.source for m in mappings))
...
def kc_conflicts(kc_id: str):
    ...
    if not any(k.id == kc_id for k in components):
        raise HTTPException(status_code=404, detail="Knowledge component not found")
    # Current KC model has one executable topic; conflicts are returned as an empty
    # result until a KC is explicitly linked to multiple executable topics.
    topics = [t for t in db.load_curriculum()[1] if any(k.id == kc_id and k.topic_id == t.id for k in components)]
...
def canonical_plan(request: CanonicalPlanRequest):
    ...
    plan = optimize_with_kc_state(
        ...
        exams,
        profile,
        __import__("datetime").date.fromisoformat(request.start_date),
        request.days,

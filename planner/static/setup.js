(() => {
  ...
  function modal(html) {
    ...
    if(!m) return;
    m.innerHTML = `<div class="modal-card setup-modal">${html}</div>`;
  ...
  async function personalPlanner() {
    close();
    window.__plannerMode = 'personal';
    window.setView('curriculum');
    window.toast('Personal Planner opened — add your subjects and topics');
  }
  ...
  async function applyBlock(blockId, blockName) {
    try {
      ...
      const multipliers = new Map([
        ['cardio',2.2],['resp-renal',2.0],['gi',1.8],['repro-endo',2.0],['neuro',2.0],
        ['immune-blood',1.9],['msk-skin',1.8],['multisystem',1.7],['development',1.5],['biostats',1.5]
      ]);
      const boost = multipliers.get(blockId) || 2.0;
      for (const s of (snap.subjects||[])) {
        const value = Number(s.exam_weight||1);
        const next = s.id === blockId ? value * boost : value;
      ...
      window.__plannerBlock = blockName;
      await window.load();
      window.toast(`Step 1 plan ready — ${blockName} is your current block`);
    } catch(e) {
  ...
  function modePicker() {
    ...
    document.querySelector('#mode-usmle').onclick = blockPicker;
    document.querySelector('#mode-personal').onclick = personalPlanner;
  }

  window.startStep1 = modePicker;
  window.openPlannerSetup = modePicker;

  // Make the existing first-run "Build my own plan" action actually enter a usable personal mode.
  const originalSetView = window.setView;
  window.setView = function(view) {
    if(view === 'curriculum' && !(window.state?.snapshot?.topics||[]).length) {
      personalPlanner();
      return;
    }
  ...
  style.textContent = `
    .setup-modal{max-width:760px!important}
    .setup-kicker{font-size:10px;letter-spacing:.14em;font-weight:900;color:#0f766e}
    .setup-modal h2{font-size:28px;letter-spacing:-.04em;margin:8px 0}
    .setup-copy{color:#71858b;line-height:1.55;margin:0 0 18px}
    .mode-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
    .mode-choice,.block-choice{border:1px solid #dce8e6;background:#fff;text-align:left;border-radius:14px;padding:16px;display:grid;gap:7px;transition:.15s;color:#102a31}
    .mode-choice:hover,.block-choice:hover{border-color:#71beb6;background:#f5fbfa;transform:translateY(-1px);box-shadow:0 10px 24px rgba(16,42,49,.07)}
    .mode-choice strong{font-size:17px}.mode-choice span,.block-choice span{font-size:11px;color:#71858b;line-height:1.45}.mode-choice b{font-size:10px;color:#0f766e}
    .block-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:9px;max-height:55vh;overflow:auto}
    .block-choice strong{font-size:13px}.block-choice span{font-size:10px}
    .setup-actions{display:flex;gap:8px;justify-content:flex-end;margin-top:14px}
    .setup-note{margin-top:15px;padding:10px 12px;border-radius:10px;background:#f6faf9;color:#71858b;font-size:10px}
    @media(max-width:650px){.mode-grid,.block-grid{grid-template-columns:1fr}.setup-modal h2{font-size:23px}}
  `;

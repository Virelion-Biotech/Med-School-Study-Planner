(() => {
  ...
  async function addPersonalSubject(){
    const name=prompt('Subject name (e.g. Cardiology, Anatomy, Pharmacology)');if(!name)return;
    const id=name.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
    if(!id)return window.toast('Please enter a usable subject name');
    try{await api2('/subjects',{method:'POST',body:JSON.stringify({id,name,exam_weight:1,category:'Personal'})});await window.load();window.__plannerMode='personal';personalWorkspace();window.toast(`${name} added`)}catch(e){window.toast(e.message)}
  }
  async function personalWorkspace(){
    close();window.__plannerMode='personal';window.__plannerBlock=null;
    const title=document.querySelector('#title'),subtitle=document.querySelector('#subtitle'),view=document.querySelector('#view');
    if(title)title.textContent='Personal Planner';
    if(subtitle)subtitle.textContent='Build your own curriculum without USMLE assumptions.';
    document.querySelectorAll('.nav').forEach(b=>b.classList.toggle('active',b.dataset.view==='curriculum'));
    const subjectCount=(typeof subjects==='function'?subjects():[]).length;
    const topicCount=(typeof topics==='function'?topics():[]).length;
    const examCount=(typeof exams==='function'?exams():[]).length;
    view.innerHTML=`<div class="hero onboarding"><div class="hero-copy"><div class="kicker">PERSONAL MODE</div><h2>Your planner. Your curriculum. Zero board assumptions.</h2><p>Add your subjects first. Then add topics and exams. The scheduler will handle complexity, fairness, review spacing and replanning for you.</p><div class="hero-actions"><button class="btn primary big" onclick="addPersonalSubject()">+ Add my first subject</button><button class="btn secondary big" onclick="openPlannerSetup()">Switch to USMLE Step 1</button></div><small>${subjectCount} subjects · ${topicCount} topics · ${examCount} exams</small></div><div class="blueprint"><div class="blueprint-head"><strong>What the planner needs</strong><span>only 3 things</span></div><div class="blueprint-row"><span>Subjects</span><b>${subjectCount}</b></div><div class="blueprint-row"><span>Topics</span><b>${topicCount}</b></div><div class="blueprint-row"><span>Exams / deadlines</span><b>${examCount}</b></div><div class="more">Nothing is weighted until you give it context.</div></div></div>`;
  }
  window.startPersonalPlanner=personalWorkspace;

  async function applyBlock(blockId,blockName){
    try{window.toast(`Preparing Step 1 around ${blockName}…`);await api2('/setup/step1',{method:'POST',body:JSON.stringify({start_date:new Date().toISOString().slice(0,10)})});const snap=await api2('/snapshot');const multipliers=new Map([['cardio',2.2],['resp-renal',2.0],['gi',1.8],['repro-endo',2.0],['neuro',2.0],['immune-blood',1.9],['msk-skin',1.8],['multisystem',1.7],['development',1.5],['biostats',1.5]]);const boost=multipliers.get(blockId)||1;for(const s of (snap.subjects||[])){const base=Number(s.exam_weight||1);const next=s.id===blockId?base*boost:base;await api2('/subjects',{method:'POST',body:JSON.stringify({id:s.id,name:s.name,exam_weight:next,category:s.category})})}await api2('/replan',{method:'POST',body:JSON.stringify({start_date:new Date().toISOString().slice(0,10),days:7,optimizer:true,locked_session_ids:[]})});close();window.__plannerMode='usmle';window.__plannerBlock=blockName;await window.load();window.toast(`Step 1 plan ready — ${blockName} is your current block`)}catch(e){window.toast(e.message)}
  }
  function blockPicker(){modal(`<div class="setup-kicker">USMLE STEP 1</div><h2>Which block are you studying right now?</h2><p class="setup-copy">The official Step 1 blueprint stays active in the background. Your current block gets extra priority so the planner reflects what you are actually studying this week.</p><div class="block-grid">${blocks.map(([id,name,desc])=>`<button class="block-choice" data-block="${id}" data-name="${esc2(name)}"><strong>${esc2(name)}</strong><span>${esc2(desc)}</span></button>`).join('')}</div><div class="setup-actions"><button class="btn secondary" id="setup-back">Back</button><button class="btn ghost" id="no-block">No block / balanced Step 1</button></div>`);document.querySelectorAll('.block-choice').forEach(b=>b.onclick=()=>applyBlock(b.dataset.block,b.dataset.name));document.querySelector('#setup-back').onclick=modePicker;document.querySelector('#no-block').onclick=()=>applyBlock('','balanced Step 1')}
  function modePicker(){modal(`<div class="setup-kicker">WELCOME</div><h2>How do you want to use your planner?</h2><p class="setup-copy">Choose the path that matches how you study. You can switch later.</p><div class="mode-grid"><button class="mode-choice" id="mode-usmle"><strong>USMLE Step 1</strong><span>Preloaded official blueprint + current block priority + adaptive scheduling.</span><b>Recommended for board prep →</b></button><button class="mode-choice" id="mode-personal"><strong>Personal Planner</strong><span>Build your own curriculum, exams, deadlines and study blocks without USMLE assumptions.</span><b>Build my own plan →</b></button></div><div class="setup-note">The planner never assumes your school curriculum matches Step 1 exactly.</div>`);document.querySelector('#mode-usmle').onclick=blockPicker;document.querySelector('#mode-personal').onclick=personalWorkspace}
  ...
  window.personalWorkspace=personalWorkspace;window.addPersonalSubject=addPersonalSubject;

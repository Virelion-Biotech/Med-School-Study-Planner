/* Local-first API adapter. It activates only when the remote backend is unavailable or clearly unsupported. */
(() => {
  if (window.__plannerLocalAdapterInstalled) return;
  window.__plannerLocalAdapterInstalled = true;
  ...
  const handle=async(input,init={})=>{const raw=typeof input==='string'?input:(input&&input.url)||'',path=raw.replace(/^https?:\/\/[^/]+/,'').split('?')[0]||'/',method=String(init.method||'GET').toUpperCase();let body={};try{body=init.body?JSON.parse(init.body):{};}catch{}const s=read();
    ...
    if(path.startsWith('/exams/')&&method==='DELETE'){const id=decodeURIComponent(path.split('/')[2]);s.exams=s.exams.filter(x=>x.id!==id);s.exams=s.exams.filter(x=>x.id!==id);write(s);return json({status:'deleted',id});}
  ...
  window.fetch=async(input,init={})=>{if(!isPlanner(input))return original(input,init);try{const response=await original(input,init);if(response && (response.ok || ![404,405,500,502,503,504].includes(response.status)))return response;const fallback=await handle(input,init);if(fallback.status!==501)return fallback;return response;}catch(error){const fallback=await handle(input,init);if(fallback.status!==501)return fallback;throw error;}};
})();

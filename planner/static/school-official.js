(() => {
  ...
  function yearPicker(id) {
    ...
    const m=shell(`<div class="setup-kicker">${esc(s.name)}</div><h2>Which year are you in?</h2><p class="school-copy">Pick your year.</p><div class="level-grid">${s.years.map(y => `<button class="mode-choice" type="button" data-year="${y}"><strong>${y==='prep'?'Preparatory Year':`Year ${y}`}</strong><span>Continue to course selection</span><b>Select →</b></button>`).join('')}</div><button class="btn ghost school-back" id="school-back" type="button">← Back</button>`);
  ...
  async function buildSchool(id, year, block) {
    ...
    try {
      ...
      const result = await api('/setup/school', {method:'POST', body:JSON.stringify({school_id:id, school_name:s.name, level:year, block})});
      close();
      if (typeof window.load==='function') await window.load();
      toast(result?.message || `${block} is now your focus`);
    } catch (e) {
      // If the deployed backend is older and does not expose the school route,
      // keep the student's selection locally and let the normal planner retry.
      if (e.status === 404 || e.status === 405) {
        close();
        toast(`${block} selected. Connect the planner backend to generate the saved schedule.`);
      } else {
  ...
  style.textContent='.school-official{width:min(920px,100%);max-width:920px!important;max-height:min(90vh,780px);overflow:auto;position:relative}.school-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin:16px 0}.school-card{display:flex;align-items:center;gap:14px;width:100%;min-height:112px;border:1px solid #dce8e6;background:#fff;border-radius:14px;padding:14px;text-align:left;cursor:pointer}.school-card:hover,.school-card:focus-visible{border-color:#71beb6;background:#f5fbfa;outline:none}.school-mark{width:58px;height:58px;min-width:58px;border-radius:12px;display:grid;place-items:center;background:#edf5f4;color:#174b50;font-weight:900;font-size:13px;border:1px solid #d5e7e4}.school-card-copy{display:grid;gap:5px;min-width:0}.school-card-copy strong{font-size:14px;color:#18343a}.school-card-copy span{font-size:10px;color:#71858b}.school-card-copy b{font-size:10px;color:#0f766e}.modal-close{position:absolute;top:12px;right:12px;width:38px;height:38px;border-radius:10px;border:1px solid #dce8e6;background:#fff;color:#486167;font-size:22px;line-height:1;cursor:pointer;z-index:2}.school-back{margin-top:4px}@media(max-width:700px){.school-list{grid-template-columns:1fr}.school-official{max-height:calc(100dvh - 16px);padding:16px}.school-card{min-height:88px}.school-mark{width:50px;height:50px;min-width:50px}}';
